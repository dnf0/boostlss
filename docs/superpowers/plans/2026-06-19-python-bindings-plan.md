# Python Bindings (PyO3) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Provide Python bindings for `boostlss` to expose the core modeling engine and learners natively in Python.

**Architecture:** Create a new Rust crate `boostlss-py` that uses `pyo3` to wrap the `boostlss` engine logic. Expose a `BoostLssModel` Python class that holds the underlying `BoostLss` engine state and config. Users will instantiate it, define the family, attach base learners for parameters, and call `.fit(X, y)`.

**Tech Stack:** Rust (2021), PyO3, Maturin, numpy (Python).
...
- [ ] **Step 2: Create `boostlss-py` Cargo.toml**
Create `crates/boostlss-py/Cargo.toml`:
```toml
[package]
name = "boostlss-py"
...
- [ ] **Step 4: Create a placeholder lib file**
Create `crates/boostlss-py/src/lib.rs`:
```rust
use pyo3::prelude::*;
...
- [ ] **Step 5: Test the library compilation**
...
- [ ] **Step 1: Create python bindings for Families**
Create `crates/boostlss-py/src/family.rs`:
```rust
use pyo3::prelude::*;
...
- [ ] **Step 2: Create python bindings for Base Learners**
Create `crates/boostlss-py/src/learner.rs`:
```rust
use pyo3::prelude::*;
...
- [ ] **Step 3: Register classes in module**
Modify `crates/boostlss-py/src/lib.rs`:
```rust
mod family;
mod learner;
...
- [ ] **Step 1: Create BoostLssModel class**
Create `crates/boostlss-py/src/model.rs`:
```rust
use pyo3::prelude::*;
