...
def run_comparison():
    ...
    for n_samples in sizes:
        ...
        print(
            f"  [BoostLSS] Fit: {blss_fit_time:.3f}s | Predict: {blss_pred_time:.3f}s"
        )

        speedup_fit = ngb_fit_time / blss_fit_time
        speedup_pred = ngb_pred_time / blss_pred_time
        print(
            f"  --> BoostLSS is {speedup_fit:.1f}x faster at fitting and {speedup_pred:.1f}x faster at predicting!"
        )
